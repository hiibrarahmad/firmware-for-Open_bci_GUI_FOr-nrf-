...
void OpenBCI_32bit_Library::beginDebug(uint32_t baudRate)
{
  ...
  {
    Serial.println("Board err");
    sendEOT();
  }
}

/**
* @description Called in every `loop()` and checks `Serial`
...
//-----------------------------------------------------------------------------------
// On XIAO/mbed we do not have setIntVector, setIntPriority, etc.
// Stub it out:
void OpenBCI_32bit_Library::boardBeginADSInterrupt() {
  #if defined(__PIC32MX__)
    setIntVector(_EXTERNAL_4_VECTOR, ADS_DRDY_Service);
    setIntPriority(_EXTERNAL_4_VECTOR, 4, 0);
...
/**
 * Used to safely end a Serial port
 */
void OpenBCI_32bit_Library::endSerial(void)
{
  if (Serial)
    Serial.end();
  iSerial.tx = false;
  ...
  if ((bufferBLE + ringBufBLETail)->flushing)
  {
    ...
    for (int i = 0; i < 4; i++)
    {
      ...
      default:
        ...
        {
          Serial.print("\nCritical error bF: ");
Serial.println((bufferBLE + ringBufBLETail)->bytesFlushed);
        }
        bufferBLEReset((bufferBLE + ringBufBLETail));
        ringBufBLETail++;
        if (ringBufBLETail >= BLE_RING_BUFFER_SIZE)
...
void OpenBCI_32bit_Library::sendChannelDataSerial(PACKET_TYPE packetType)
{
  ...
  writeSerial(PCKT_START);   // 1 byte - 0x41
...
pinMode(ADS_DRDY, INPUT_PULLUP);    // XIAO DRDY
delay(40);

resetADS(BOARD_ADS);                // only on‐board ADS
delay(10);
// Tell the ADS to use 250 SPS, but do NOT try daisy-clk
WREG(CONFIG1, (ADS1299_CONFIG1_DAISY_NOT | curSampleRate), BOARD_ADS);
numChannels = OPENBCI_NUMBER_OF_CHANNELS_DEFAULT;
...
/**
* @description Used to report (Serial.print) the default channel settings
...
//         if (targetSS == BOARD_ADS) {
//             Serial1.print("Set channel "); Serial1.print(channelNumber); Serial.println(" settings");
//         }
//     }
...
{
    // after writing CONFIG2 we’ll read it back and print so you can see exactly what landed in the ADS
  auto debugPrintConfig2 = [&](int ss, byte value) {
    // print which ADS we just configured
    printAll((ss == BOARD_ADS) ? "CONFIG2@BOARD_ADS = " : "CONFIG2@DAISY_ADS = ");
    // print the hex value
    printHex(value);
    // end‐of‐text so it flushes nicely
    sendEOT();
};
  byte setting, targetSS;
  for (int i = 0; i < 2; i++)
  {
    ...
    byte newConfig2 = 0b11010000 | freqCode | amplitudeCode;
    WREG(CONFIG2, newConfig2, targetSS);
    delay(1);
    // now read it back and dump it to your serial console
    byte confirmed = RREG(CONFIG2, targetSS);
    debugPrintConfig2(targetSS, confirmed);
  }
}
...
void OpenBCI_32bit_Library::updateChannelData(void)
{
  ...
  if (iSerial.tx == false && iSerial1.baudRate > OPENBCI_BAUD_RATE_MIN_NO_AVG)
  {
    downsample = false;
  }
  ...
  {
  case BOARD_MODE_ANALOG:
    auxData[1] = 0;  // analogRead(A6) on PIC32 → stub out on XIAO
    auxData[2] = 0;  // analogRead(A7) on PIC32 → stub out on XIAO
    if (!wifi.present)
    {
      auxData[2] = 0;
    }
...
{
  if (iSerial.tx || (iSerial1.tx && iSerial1.baudRate <= OPENBCI_BAUD_RATE_MIN_NO_AVG))
  {
    if (daisyPresent)
    {
